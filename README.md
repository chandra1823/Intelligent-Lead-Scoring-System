...
- a **training script** for generating a baseline ML model,
- and **unit tests**.
...
## 7) Train your own ML model artifact (optional but recommended)

By default, if no trained artifact exists, fallback logic is used for demo continuity.
...
Now `/predict` uses the trained artifact when available.
